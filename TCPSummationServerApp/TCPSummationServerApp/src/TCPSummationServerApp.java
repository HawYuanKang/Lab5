	...
	public static void main(String[] args) {
		...
		try {
			...
			while (true) {
				...
				// Further processing
				int totalS = numberCalculator.getSum(number1, number2, number3);
				int totalM = numberCalculator.getMulti(number1, number2, number3);
				
				// 5. Respond to client - return total to client
				OutputStream outStream = clientRequest.getOutputStream();
				DataOutputStream dos = new DataOutputStream(outStream);
				dos.writeInt(totalS);
				System.out.println("\tTo client Sum: " + totalS);
				dos.writeInt(totalM);
				System.out.println("\tTo client Multi: " + totalM);
				// To client: 2101
